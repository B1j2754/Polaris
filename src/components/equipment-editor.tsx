...
export function EquipmentEditor({ value, onChange, emptyText }: Props) {
  ...
  return (
    <View className="w-full gap-3">
      ...
      {scope && camera && (
        <Text className="font-sansation px-2 text-center text-sm text-fg-dim">{fieldOfView(scope, camera)}</Text>
      )}
      ...
      <View className="flex-row justify-center gap-3">
        {(["telescope", "camera"] as Kind[]).map((kind) => (
          ...
          >
            ...
            <Text className="font-sansation text-lg text-fg">{kind === "telescope" ? "Telescope" : "Camera"}</Text>
          </Pressable>
        ))}
      </View>
